...
# ///
"""Демо: входящие заявки сервисной компании, правила потом модель.

Дешёвые правила снимают очевидный мусор. Модель смотрит только выживших
и раскладывает по исходам: менеджеру, в календарь, в рассылку, отказ.
Спорные (нет уверенности или цитата не из текста) - отдельно человеку.

    ./app.py                 # локально на :8080
    ./app.py --selftest      # механика и сборка, без вызова модели
    ./app.py --record        # один раз записать мнения модели в data/judgments.json
"""
...
# Прейскурант gemini-2.5-flash, USD за токен. Источник: ai.google.dev/gemini-api/docs/pricing,
# сверено 23.08.2026 ($0,30 за 1M входных, $2,50 за 1M выходных).
...
# --- Правила. Порядок = сначала мусор, потом суть заявки ------------------------
RULE_LABELS = {
...
def sentence_with(text: str, needle: str) -> str:
    ...
    # Фраза без точки - вырежем окно вокруг совпадения.
...
def after_hours_hit(text: str, start: int, end: int) -> tuple[bool, str]:
    """Просьба приехать вне смены или в выходной. Цитата - из текста."""
    ...
    if weekend and hour is not None:
        # Выходной в любой названный час - вне обычной смены.
    ...
    if tonight and hour is None:
        # «tonight» без часа для выезда всё равно вне дневной смены.
...
def screen_one(req: dict, c: dict, seen: dict[tuple[str, str], str]) -> dict:
    """Первое сработавшее правило снимает заявку. Без модели."""
    text = req["text"]
    client = req.get("client") or {}
...
# --- Провайдер: Vertex trial, потом GEMINI_API_KEY_FREE. OpenRouter не трогать ---
...
def model_client():
    """Клиент модели. В контейнере креды из env, локально - accounts.json, потом free key."""
    today = datetime.date.today().isoformat()
    if today in _client_cache:
        return _client_cache[today]
    # Под локом, с повторной проверкой: параллельные вызовы без него
    # каждый создавал свой клиент и затирал чужой через clear().
    with _client_lock:
        if today in _client_cache:
            return _client_cache[today]
...
def _build_client(today: str):
    ...
    raise RuntimeError(
        "нет живого Vertex и нет GEMINI_API_KEY_FREE"
...
def _is_exhausted(e: Exception) -> bool:
    """429 / RESOURCE_EXHAUSTED / quota - ступень выгорела, не настоящая поломка."""
...
def free_client():
    """Последняя бесплатная ступень: GEMINI_API_KEY_FREE. Платный OpenRouter не трогаем."""
    ...
    with _free_lock:
        ...
        key = _env("GEMINI_API_KEY_FREE")
        if not key:
            raise RuntimeError("Vertex исчерпан, а GEMINI_API_KEY_FREE нет")
        _client_cache["free"] = genai.Client(api_key=key)
        return _client_cache["free"]


def judge(req: dict, criteria: dict) -> dict:
    """Один вызов модели на одну заявку."""
    ...
    try:
        ...
        # Квота ступени кончилась - спускаемся ниже, а не падаем.
...
def finalize_model_row(cheap_row: dict, req_row: dict, j: dict, criteria: dict) -> dict:
    """Мнение модели + гейт цитаты и порога уверенности → исход или спорное."""
...
def assemble(criteria: dict) -> dict | None:
    """Живая механика + записанные мнения модели. Модель не зовётся."""
    rec = load_recording()
    if rec is None:
        return None
...
def record_judgments(*, force: bool = False) -> None:
    """Последовательная запись. Параллель пробивает бесплатный лимит 5/мин."""
    rec = {
...
# --- Selftest: механика, ни одного вызова модели --------------------------------
EXPECTED_RULE = {
    "r01": "spam_or_ads",
...
def selftest() -> None:
    ...
    assert len(REQUESTS) == 40, f"ждали 40 заявок, получили {len(REQUESTS)}"
    ...
    for rid, rule in EXPECTED_RULE.items():
        ...
        assert got == rule, f"{rid}: ждали {rule!r}, получили {got!r}"
        if rule:
            quote = results[rid]["quote"]
            assert quote, f"{rid}: пустая цитата"
            assert quote_in_body(quote, REQ_BY_ID[rid]["text"]), (
                f"{rid}: цитата не из текста: {quote!r}")
    ...
    assert len(survivors) == 11, f"выживших {len(survivors)}, ждали 11"
    assert len(dropped) == 29, f"снятых {len(dropped)}, ждали 29"
    ...
    # Пороги на странице обязаны менять результат.
    wide_area = dict(c, service_area=list(c["service_area"]) + ["Cedar Falls"])
    assert screen_all([REQ_BY_ID["r03"]], wide_area)[0]["rule"] is None, (
        "добавили Cedar Falls в зону - Nora должна выжить")

    cheap_min = dict(c, min_order_usd=30)
    assert screen_all([REQ_BY_ID["r06"]], cheap_min)[0]["rule"] is None, (
        "снизили минимум до 30 - Sam должен выжить")

    long_hours = dict(c, work_hours_start=0, work_hours_end=24)
    # 10pm попадает в 0-24, но «tonight» + выезд всё ещё может считаться ночным;
    # для круглосуточной смены after_hours_hit по часу не сработает.
    r07 = screen_all([REQ_BY_ID["r07"]], long_hours)[0]
    assert r07["rule"] is None, f"круглосуточно - Jules должна выжить, получили {r07['rule']}"

    # Без доплаты Sunday 10am - после расширения зоны/часов r39 жив; без surcharge ок.
    ...
    # Снятые правилом лежат в decline с именем правила и цитатой из текста.
